...
**开工分支：**  
- `implementation/issue-15-tee-junction-pipe-geometry-mesh`
...
### 创建缘由
根据天工3月项目需求，液压管件流固耦合优化场景中的几何建模模块需要生成三通管的几何模型，并直接通过gmsh进行网格生成。此模块将几何建模与网格生成结合在一起，以提高开发效率，确保满足后续流固耦合计算的精度要求。

### 当前承载问题
- 需要确保gmsh生成的三通管几何模型与网格划分符合后续计算需求。
- 缺少区分管壁与流体域的标记方法，需确保固体域与流体域能够正确划分。

### 目标
- 开发三通管的几何建模与网格生成模块，实现从参数输入到几何模型与网格输出的全过程。
...
### 工作范围
- 实现三通管的几何建模与网格生成模块，利用gmsh结合几何与网格生成步骤。
...
### 完成定义（DoD）
- 在 `fealpy/geometry` 模块中完成三通管的几何建模与网格生成程序开发，能够根据设计输入生成目标几何模型并生成网格。
- 在网格中，区分管壁为固体域、管道内部为流体域，且为每个区域设置不同标签。
- 完成模块文档，包括接口设计、参数定义及使用指南，文档路径：`docs/design/hydraulic_pipe_fsi_optimization/fealpy_tee_pipe_mesher_user_manual_zh.md`。
...
### 建议下一动作：
- 开始开发 `BaseTeeJunctionPipeGeometry` 类，进行初步的几何与网格生成测试。
