...
def test_cylinder_grid_convergence_runs_fealpy_levels(tmp_path, monkeypatch):
    ...
    def fake_run_piso_cylinder(case_args):
        calls.append(
            (
                case_args.mesh_size,
        ...
        return object(), {
            ...
            "summary": {
                ...
                "iterations": case_args.nt,
                "force_drag_coefficient": value,
                "pressure_drop_delta_p": 0.1 + case_args.mesh_size,
            },
        }

    monkeypatch.setattr(conv, "run_piso_cylinder", fake_run_piso_cylinder)
    ...
    assert "NSFVMPISOModel" not in summary_text
...
def test_successive_observed_orders_use_three_grid_levels():
    convergence_rows = load_piso_example().grid_convergence_rows
